...
void CompareTrainings(bool save_plots = false) {
  ...
  std::string datadir = "/exp/mu2e/data/users/edmonds/TrkQualTraining/";
  ...
  std::string datasets[n_versions] = {"", "_training"}; // for v1.1. and earlier, we did not store infidivual training, validation, mock results
  ...
  for (int i_version = 0; i_version < n_versions; ++i_version) {
    ...
    std::string dataset = datasets[i_version];
    std::string filename = datadir + "/TrkQual_ANN1_" + version + "_plots.root";
    ...
    c_mom_res->cd();
    std::string histname = "all_mom_res" + dataset;
    ...
    for (int i_feature = 0; i_feature < n_features; ++i_feature) {
      ...
      std::string histname = "high_qual_feature" + std::to_string(i_feature) + "_" + features[i_feature] + dataset + "_norm";
      ...
      histname = "low_qual_feature" + std::to_string(i_feature) + "_" + features[i_feature] + dataset +  "_norm";
